...
![海洋、森林、赛车、珠宝、书架与人物表演的网页效果截图导览](assets/effect-guide.svg)

以原展厅的 6 张网页预览截图作为导览和项目封面，先看具体画面，再进入原作或下方 80 个案例。截图保持原画面比例，图注说明参考细节与原作名称。
...
检查 80 条数据唯一性与必填字段、原目录来源一致性、80 张图片、静态链接与锚点、JavaScript 语法、各分类及复位逻辑、元数据索引和 HTTP 入口。截图导览更新已通过桌面与 390px 窄屏浏览器检查，6 张导览图片加载正常，窄屏无横向溢出；封面拼排已目视核对。原有原作操作验证与本地静态检查分别记录。

未验证所有原作的源码、AI 调用、生成成本、保存导出、多人同步、性能或实际收益。由 AI 制作不代表运行时依赖模型。

## 来源

案例简介按展厅信息归纳，预览图来自 Awesome Sites，具体地址见目录快照。图片不是本次生成或复现成果；原作与预览归各自权利人所有。中文参考点、截图导览编排和展示页为原创研究整理，未复制或重新托管原作源码。
