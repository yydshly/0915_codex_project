...
import html
import shutil
from projects import ROOT, projects
...
def build():
    ...
    for item in projects():
        ...
        cards.append(f'<article><a href="{name}/">{cover}</a><div><span>RESEARCH / {item["id"]}</span><h2><a href="{name}/">{title}</a></h2><p>{summary}</p><a class="go" href="{name}/">阅读研究 →</a> <a class="upstream" href="{upstream}">原始来源 ↗</a></div></article>')
        rows.append(f'<tr><td>{item["id"]}</td><th><a href="{name}/">{title}</a></th><td>{summary}</td><td><a href="{upstream}">原始来源 ↗</a></td></tr>')
